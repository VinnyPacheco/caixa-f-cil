...
import { Transaction, TransactionWithBalance, MonthSummary, Category, Account } from '@/types/finance';
import { 
  fetchTransactions,
  createTransaction, 
  updateTransaction, 
...
export function useTransactionsData(selectedDate: Date) {
  const { user } = useAuth();
  ...
  const accountsQuery = useQuery({
    queryKey: ['accounts'],
    queryFn: async () => {
      const { fetchAccounts } = await import('@/services/accountsService');
  ...
  const transactionsWithBalance: TransactionWithBalance[] = useMemo(() => {
    const withRelations = filteredTransactions.map((t) => ({
      ...
      account: accounts.find((a: Account) => a.id === t.accountId),
      runningBalance: 0,
    }));
